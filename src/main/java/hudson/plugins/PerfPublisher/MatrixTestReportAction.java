...
public class MatrixTestReportAction extends AbstractPerfPublisherAction
    ...
    public void computeStats() {
        ...
        for (PerfPublisherMatrixSubBuild subBuild : subBuilds) {
            this.numberOfExecutedTest += subBuild.getReport().getNumberOfExecutedTest();
            this.numberOfFailedTest += subBuild.getReport().getNumberOfFailedTest();
            this.numberOfTest += subBuild.getReport().getNumberOfTest();
            this.numberOfPassedTest += subBuild.getReport().getNumberOfPassedTest();
            this.numberOfTrueFalseTest += subBuild.getReport().getNumberOfTrueFalseTest();
        }
    ...
    public String getDetailSummary() {
        String strbuilder = ("Number of executed tests : <b>" + this.getNumberOfExecutedTest() + ".</b>") +
                "<br />";
...
//	}
    ...
    public String getHtmlArrayDisplay() {
        String strb = "<table class=\"matrix_table\">\n" +
                generateHtmlArrayHeader() +
                generateHtmlArrayContent() +
                "</table>";
        /*
    ...
    private String generateHtmlArrayContent() {
        ...
        for (PerfPublisherMatrixSubBuild subBuild : subBuilds) {
            if (values.containsKey("Nb tests")) {
                ...
                nbs.add(subBuild.getReport().getNumberOfTest());
            } else {
            ...
            if (values.containsKey("Nb Executed tests")) {
                ...
                nbs.add(subBuild.getReport().getNumberOfExecutedTest());
            } else {
                List<Integer> nbs = new ArrayList<Integer>();
                nbs.add(subBuild.getReport().getNumberOfExecutedTest());
            ...
            if (values.containsKey("Nb not executed tests")) {
                ...
                nbs.add(subBuild.getReport().getNumberOfNotExecutedTest());
            } else {
                List<Integer> nbs = new ArrayList<Integer>();
                nbs.add(subBuild.getReport().getNumberOfNotExecutedTest());
            ...
            if (values.containsKey("Succeeded tests")) {
                ...
                nbs.add(subBuild.getReport().getNumberOfPassedTest());
            } else {
                List<Integer> nbs = new ArrayList<Integer>();
                nbs.add(subBuild.getReport().getNumberOfPassedTest());
            ...
            if (values.containsKey("Failed tests")) {
                ...
                nbs.add(subBuild.getReport().getNumberOfFailedTest());
            } else {
                List<Integer> nbs = new ArrayList<Integer>();
                nbs.add(subBuild.getReport().getNumberOfFailedTest());
            ...
            if (values.containsKey("Avg Compile time")) {
                ...
                nbs.add((int) subBuild.getReport().getAverageOfCompileTime());
            } else {
                List<Integer> nbs = new ArrayList<Integer>();
                nbs.add((int) subBuild.getReport().getAverageOfCompileTime());
            ...
            if (values.containsKey("Avg Execution time")) {
                ...
                nbs.add((int) subBuild.getReport().getAverageOfExecutionTime());
            } else {
                List<Integer> nbs = new ArrayList<Integer>();
                nbs.add((int) subBuild.getReport().getAverageOfExecutionTime());
            ...
            if (values.containsKey("Avg Performance")) {
                ...
                nbs.add((int) subBuild.getReport().getAverageOfPerformance());
            } else {
                List<Integer> nbs = new ArrayList<Integer>();
                nbs.add((int) subBuild.getReport().getAverageOfPerformance());
        ...
        for (int i_row = 0; i_row < this.matrixbuild.getNbAxis(); i_row++) {
            ...
            for (int i_factor = 0; i_factor < factor; i_factor++) {
                for (String value : values) {
                    if (colspan > 1) {
                        strb.append("<td COLSPAN=" + colspan + " class=\"axe_value\">" + value + "</td>");
                    } else {
                        strb.append("<td class=\"axe_value\">" + value + "</td>\n");
                    }
                }
            }
            factor *= values.size();
