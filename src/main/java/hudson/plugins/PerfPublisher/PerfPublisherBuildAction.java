...
public class PerfPublisherBuildAction extends AbstractPerfPublisherAction
    ...
    public String getHtmlListOfBuildsInOptions() {
        ...
        for (Run build : getBuilds()) {
            if (!build.isBuilding()
                    && build.getResult().isBetterOrEqualTo(
                    Result.FAILURE)) {
                PerfPublisherBuildAction ac = build
                        .getAction(PerfPublisherBuildAction.class);
                if (ac != null) {
                    strbuilder.append("<option value=\"").append(build.getNumber()).append("\"");
                    strbuilder.append(">").append(build.getNumber()).append("</option>\n");
                }
            }
        }
...
//        return this.build.getProject().getBuilds();
    ...
    public String getHtmlListOfBuildsInOptionsWithSelected() {
        ...
        for (Run build : getBuilds()) {
            if (!build.isBuilding()
                ...
                if (ac != null) {
                    strbuilder.append("<option value=\"").append(build.getNumber()).append("\"");
                    if (build.getNumber() == this.build.getNumber()) {
    ...
    public String getDetailSummary() {
        String strbuilder = ("Number of parsed files : <b>"
                + reports.getNumberOfFiles() + ".</b>") +
                "<br />" +
                "Number of executed tests : <b>"
    ...
    public String getRegression() {
        ...
        if (this.getTrendReport() != null) {
            ...
            for (Test tmpTest : tmpTests) {
                if (!tmpTest.isSuccessfull()) {
                    regressions.add(tmpTest);
                }
            }
        ...
        if (link.startsWith("testDetails.")) {
            ...
            if (request.getParameter("build1") != null
                ...
                if (!request.getParameter("build1").equals("none")) {
                    ...
                    build1 = getBuildByNumber(nb_build1);
                ...
                if (!request.getParameter("build2").equals("none")) {
                    ...
                    build2 = getBuildByNumber(nb_build2);
                ...
                if (!request.getParameter("build3").equals("none")) {
                    ...
                    build3 = getBuildByNumber(nb_build3);
